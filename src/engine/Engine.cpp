...
void Engine::updateToCurrentDQNState( State &stateToUpdate )
{
    ...
    for ( const auto &plConstraint : _plConstraints )
    {
        ...
        if ( !plConstraint->isActive() )
            phase = RELU_PHASE_ACTIVE; // todo check
        stateToUpdate.updateConstraintPhase( index, static_cast<int>( phase ) );
        // if phase fixed : relu constraint's current variable = _f. _b otherwise . // todo check
        const auto variable = plConstraint->phaseFixed()
                                ? plConstraint->getParticipatingVariables().back()
                                : plConstraint->getParticipatingVariables().front();
    ...
    while ( true )
    {
        ...
        try
        {
            ...
            if ( _smtCore.needToSplit() )
            {
                if ( GlobalConfiguration::USE_DQN )
                {
                    PhaseStatus phaseStatus = valueToPhase( _action->getAssignmentStatus() );
                                              ...
                                              const std::string &trainedAgentPath )
{
    ...
    if ( _agent->getReplayBufferSize() == 0 )
    {
        printf( "empty replayBuffer!\n" );
        fflush( stdout );
    }
    else
    {
        printf( "not empty! : %d\n", _agent->getReplayBufferSize() );
        fflush( stdout );
    }
        ...
        if ( shouldExitDueToTimeout( timeoutInSeconds ) )
        {
            ...
            _agent->step( previousState,
                          ...
                          *_currentDQNState,
                          ...
                          numSplitsByAgent,
                          false );
            _agent->handleDone( *_currentDQNState, _smtCore.getStackDepth(), numSplitsByAgent );
                ...
                while ( !smtSteps.empty() )
                {
                    ...
                    if ( smtStep == 1 )
                    {
                        ...
                        _agent->addAlternativeAction(
                            *_currentDQNState, stackDepth, numSplitsByAgent, numInconsistent );
                    }
                    else if ( smtStep == 2 )
                    {
            ...
            if ( allVarsWithinBounds() )
            {
                ...
                if ( solutionFound )
                {
                    ...
                    if ( allNonlinearConstraintsHold() )
                    {
                        ...
                        auto rewardForDone = _smtCore.discoveredSubtrees( numPlConstraints() );
                        rewardForDone = std::copysign(
                            std::log( 1.0 + std::abs( rewardForDone ) / 10.0 + 1e-8 ),
                            rewardForDone );
                        ...
                        _agent->step( previousState,
                                      ...
                                      *_currentDQNState,
                                      ...
                                      false );
                        _agent->handleDone(
                            *_currentDQNState, _smtCore.getStackDepth(), numSplitsByAgent );
                        printf( "success!" );
                        fflush( stdout );
                        return std::move( _agent );
                    }
...
int Engine::findPlConstraintsIndex( const int index,
                                    List<PiecewiseLinearConstraint *> *constraints )
{
    const PiecewiseLinearConstraint *plConstraintFromIndex =
        indexToConstraint( index, &_plConstraints );
...
PiecewiseLinearConstraint *Engine::pickSplitPLConstraintByAgent()
{
    ...
    {
        printf( "trying to insert bad step\n" );
        fflush( stdout );
